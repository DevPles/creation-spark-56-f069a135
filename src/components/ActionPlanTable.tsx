      ...
      {plans.map((plan, i) => {
        ...
        return (
          <motion.div
            ...
            className={`px-5 py-3 grid grid-cols-12 items-center text-sm border-b border-border last:border-0 hover:bg-muted/30 transition-colors cursor-pointer ${isCritica ? "bg-destructive/5" : ""}`}
          >
            ...
            <span className="col-span-2 text-muted-foreground truncate text-xs">
              ...
              {plan.area && <span className="block text-[10px]">{plan.area}</span>}
            </span>
            <span className="col-span-2 text-muted-foreground truncate">{plan.responsavel || "—"}</span>
