  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="kpi-card"><p className="text-xs text-muted-foreground">Total de planos</p><p className="kpi-value">{total}</p></div>
        ...
        <div className="kpi-card"><p className="text-xs text-muted-foreground">Evidências pendentes</p><p className="kpi-value text-warning">{pendentes}</p></div>
        <div className="kpi-card"><p className="text-xs text-muted-foreground">Alta/Crítica</p><p className="kpi-value text-risk">{criticas}</p></div>
      </div>

      {/* Generate report */}
      <div className="bg-card rounded-lg border border-border p-5 space-y-4">
        <h3 className="text-sm font-semibold">Gerar Relatório Inteligente</h3>
        <p className="text-xs text-muted-foreground">
          O relatório analisa todos os planos de ação e identifica padrões, áreas críticas e recomendações priorizadas.
        </p>
        <div className="flex items-center gap-3">
          ...
          <Button onClick={generateReport} disabled={loading || total === 0}>
            {loading ? "Gerando..." : "Gerar relatório"}
          </Button>
        </div>

        {report && (
          <div className="mt-4 bg-muted/30 rounded-lg border border-border p-5">
            <h4 className="text-sm font-semibold mb-3">Relatório Consolidado</h4>
